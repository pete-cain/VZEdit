...
VzBank.prototype.init = function(sysexBuff, fileName) {
	if (sysexBuff != null)
		this.sysexArray = new Uint8Array(sysexBuff);
...
VzBank.prototype.parseOps = function() {
	...
	for (var int = 0; int < this.numOfOps; int++) {
		op = new Operation(this.getOperationArray(int));
		this.ops.push(op);
...
VzBank.prototype.writeFile = function(){
...
CainUtils.prototype.hexOutput = function(sysExBuffer) {
	...
	for (var i = 0; i < sysExBuffer.length; i++) {
		...
		if ((hex(char >>> 4) == 'F') && (hex(char & 0xF) == '7'))
			text += " <br>";
	}
};
...
function Tone(toneArray) {
	...
	origChecksum = toneArray[this.dataLength*2];
...
function Operation(opArray) {
	...
	this.xtractTones();
	origChecksum = opArray[this.dataLength*2];
	this.checkSum = this.calculateChecksum();
	this.isEmpty();
...
Operation.prototype.xtractTones = function() {
	...
	for (var i = 0; i < this.startSD.length; i++) {
		...
		this.tones.push((this.raw[startPoint * 2] * 16 + this.raw[startPoint * 2 + 1]) & 0x3f);
	}
};
