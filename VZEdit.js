...
function detectBarProximity(kfCanvas, evt) {
  ...
  for (var i = 0; i < 8; i++) {
    x =
      vzTone.getKeyFollowNote(sourceEnvelope, i) * (kfCanvas.width / 792) * 5 +
      x_offset;
    y =
      kfCanvas.height -
      (vzTone.getKeyFollowLevel(sourceEnvelope, i) * kfCanvas.height) / 792 +
      y_offset;
...
function detectNodeProximity(envCanvas, evt) {
  ...
  for (var i = 0; i < 8; i++) {
    ...
    if (sourceEnvelope == 8) {
      y =
        envCanvas.height / 2 -
        (envCanvas.height / 126) * vzTone.getEnvelopeLevel(sourceEnvelope, i);
    } else {
      y =
        envCanvas.height - vzTone.getEnvelopeLevel(sourceEnvelope, i) * yScale;
    }
    if (
...
function drawKFBars(canvas, module, barProximity) {
  ...
  for (var i = 0; i < 6; i++) {
    x =
      vzTone.getKeyFollowNote(module, i) * 5 * (canvas.width / 792) + x_offset;
    y =
      canvas.height +
      y_offset -
      (vzTone.getKeyFollowLevel(module, i) * (canvas.height + y_offset)) / 100;
    if (barProximity == i + 1) {
...
function editEnvelope(envCanvas, evt, nodeProximity) {
  ...
  for (var i = 0; i < 8; i++) {
    if (nodeProximity - 1 == i) {
      ...
      newX = Math.floor(
        (mousePos.x - (x * envCanvas.width) / 792) / (envCanvas.width / 792)
      );
      if (newX <= 0) newX = 0;
      if (newX >= 99) newX = 99;
      ...
      if (sourceEnvelope == 8) {
        vzTone.setEnvelopeLevel(
          sourceEnvelope,
          i,
          ((envCanvas.height / 2 - mousePos.y) * 126) / envCanvas.height
        );
      } else {
...
function doMouseUp(envCanvas, evt) {
  ...
  if (envMouseState == 1) {
    ...
    if (lineProximity >= 1) {
      ...
      else vzTone.setEnvelopeVelocity(sourceEnvelope, lineProximity - 1, 0);
    }
  }
  envMouseState = 0;
  kfMouseState = 0;
...
send_data_to_vz = function () {
  ...
  outport.send(SysexMessage, window.performance.now() + 2000);
};
...
function changeMidiChannel(value) {
  ...
  if (value > 15 || value < 0) value = 1; // it should throw an error... I know
