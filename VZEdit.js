...
function buildModulationType(type) {
  ...
  if (type == 'tremolo') modType = 2;
	...
	modulationString += "<input type='range' id='" + type + "Rate' min='0' max='99' step='1' value='0' onchange='setMod" + type + "()'> rate <br>";
	modulationString += "<input type='range' id='" + type + "Delay' min='0' max='99' step='1' value='0' onchange='setMod" + type + "()'> delay <br>";
...
function buildEnvelopeModules(module) {
	...
	envelopeModuleString = "<input type='submit' value='' class='unlocked'  id='envLock" + module + "' onclick = 'lockEnvelopeModule(" + module
			+ ",1)' >Envelope <br> ";
...
function toggleModuleUI(module) {
	vzTone.module[module - 1].active = (vzTone.module[module - 1].active + 1) % 2;
...
function pitchChangeRange() {
	if ($('#pitchRange').is(':checked'))
		vzTone.pitchEnvRange = 1;
	else
		vzTone.pitchEnvRange = 0;
	send_data_to_vz();
}
...
function detectBarProximity(kfCanvas, evt) {
	...
	for (var i = 0; i < 8; i++) {
		x = vzTone.getKeyFollowNote(sourceEnvelope, i) * (kfCanvas.width / 792) * 5 + x_offset;
		y = kfCanvas.height - vzTone.getKeyFollowLevel(sourceEnvelope, i) * kfCanvas.height / 792 + y_offset;
...
function detectNodeProximity(envCanvas, evt) {
	...
	for (var i = 0; i < 8; i++) {
		...
		x += time;
		if (sourceEnvelope == 8) {
			y = envCanvas.height / 2 - (envCanvas.height / 126) * vzTone.getEnvelopeLevel(sourceEnvelope, i);
		} else {
			y = envCanvas.height - vzTone.getEnvelopeLevel(sourceEnvelope, i) * yScale;
		}
...
function drawEnvelope(canvas, module, state, lineProximity) {
	...
	for (var i = 0; i < 8; i++) {
		...
		x += time;
		if (module == 8) {
			y = canvas.height / 2 - (canvas.height / 126) * vzTone.getEnvelopeLevel(module, i);
		} else {
			y = canvas.height - (canvas.height * vzTone.getEnvelopeLevel(module, i) / 100);
		}
...
function drawKFBars(canvas, module, barProximity) {
	...
	for (var i = 0; i < 6; i++) {
		x = vzTone.getKeyFollowNote(module, i) * 5 * (canvas.width / 792) + x_offset;
		y = (canvas.height + y_offset) - vzTone.getKeyFollowLevel(module, i) * (canvas.height + y_offset) / 100;
		if (barProximity == i + 1) {
		...
		context.fillRect(x - 4, y, 8, vzTone.getKeyFollowLevel(module, i) * (canvas.height + y_offset) / 100);
...
function drawKF(canvas, module, barProximity, status) {
	...
	for (var i = 0; i < 6; i++) {
		x = Math.floor(vzTone.getKeyFollowNote(module, i) * 5 * (canvas.width / 792) + x_offset);
		y = Math.floor((canvas.height + y_offset) - vzTone.getKeyFollowLevel(module, i) * (canvas.height + y_offset) / 100);
		ctx.lineTo(x, y);
	}
...
function editEnvelope(envCanvas, evt, nodeProximity) {
	...
	for (var i = 0; i < 8; i++) {
		if (nodeProximity - 1 == i) {
			...
			newX = Math.floor((mousePos.x - x * envCanvas.width / 792) / (envCanvas.width / 792));
			if (newX <= 0)
				newX = 0;
			if (newX >= 99)
				newX = 99;
			...
			if (sourceEnvelope == 8) {
				vzTone.setEnvelopeLevel(sourceEnvelope, i, ((envCanvas.height / 2 - mousePos.y) * 126 / envCanvas.height));
			} else {
...
function editKFLevel(canvas, evt, barProximity) {
	...
	if (barProximity > 0) {
		...
		newY = ((canvas.height - mousePos.y) * 100 / canvas.height + y_offset) - y_offset;
		if (newX <= left)
			newX = (left + 1);
		if (newX >= right)
			newX = (right - 1);
...
function updateEnvelopeTable() {
	for (var i = 0; i < 8; i++) {
		...
		document.getElementById('l' + (i + 1)).value = Math.floor(vzTone.getEnvelopeLevel(sourceEnvelope, i));
		document.getElementById('r' + (i + 1)).value = vzTone.getEnvelopeTime(sourceEnvelope, i);
...
function updateKFTable() {
	...
	for (var i = 0; i < 6; i++) {
		...
		document.getElementById('kf_level' + (i + 1)).value = Math.floor(vzTone.getKeyFollowLevel(sourceEnvelope, i));
		document.getElementById('kf_note' + (i + 1)).value = notes[vzTone.getKeyFollowNote(sourceEnvelope, i) % 12]
...
function updateEnvelope(envCanvas, lineProximity) {
	...
	if (sourceEnvelope != 8) {
		for (i = 0; i < 8; i++) {
			if (lock[1][i] == 1)
				vzTone.copyEnv(sourceEnvelope, i);
		}
	}
...
function doKFMouseMove(kfCanvas, evt) {
	...
	for (var i = 0; i < 8; i++) {
		if (lock[2][i] == 1)
			vzTone.copyKF(sourceEnvelope, i);
	}
	for (var i = 0; i < 8; i++) {
		status = 0;
		if (lock[2][i] == 1)
...
function doMouseUp(envCanvas, evt) {
	...
	if (envMouseState == 1) {
		...
		if (lineProximity >= 1) {
			if (vzTone.getEnvelopeVelocity(sourceEnvelope, lineProximity - 1) == 0)
				vzTone.setEnvelopeVelocity(sourceEnvelope, lineProximity - 1, 1);
			else
				vzTone.setEnvelopeVelocity(sourceEnvelope, lineProximity - 1, 0);
		}
	}
	envMouseState = 0;
	kfMouseState = 0;
...
fillSysexToneData = function() {
	vzTone.getHexArray();
	var toneData = [];
	for (var i = 0; i < internalToneData.length; i++) {
		if (internalToneData[i] < 0)
			console.log("--->" + i + ":" + internalToneData[i]);
		if (internalToneData[i] > 255)
			console.log("too hi! " + i + ":" + internalToneData[i]);
		toneData.push(internalToneData[i] >> 4);
		toneData.push(internalToneData[i] & 0x0f);
	}
	return toneData;
};
...
function selectFixedPitch(module) {
	internalToneData[3 + 2 * module] &= 0xfd;
	if ($('#det_pitch_fixM' + module).is(':checked'))
		internalToneData[3 + 2 * module] += 1 << 1;
	send_data_to_vz();
}
...
function setDetune(module) {
	...
	document.getElementById('oct_M' + module).value = vzTone.getDetuneOctave(module - 1);
	document.getElementById('note_M' + module).value = vzTone.getDetuneNote(module - 1);
	document.getElementById("cent_M" + module).value = vzTone.getDetuneCent(module - 1);
